...
export default function FridgeScreen() {
  ...
  const handleGenerate = async () => {
    if (cart.length === 0) {
      Alert.alert(
        "Empty Pot",
        "Add some ingredients before generating a recipe.",
      );
      return;
    }
    setGenError(null);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const recipeName =
        lines[0]?.replace(/^(recipe:|name:)/i, "").trim() || "Untitled Recipe";
      await saveRecipe({
        recipeName,
        ingredients: cart.map((c) => ({
          name: c.name,
          emoji: c.emoji,
          quantity: c.qty,
        })),
        ...
        <View style={styles.fridgePanel}>
            ...
            {CATEGORIES.map((cat) => (
              ...
              >
                ...
                <Text
                  style={[
                    styles.tabLabel,
                    activeCatId === cat.id && { color: cat.color },
                  ]}
                >
                  {cat.label}
                </Text>
          ...
          {(activeCategory || showGlobal) && (
            <View
              style={[
                styles.catBanner,
                { borderLeftColor: activeCategory?.color ?? "#94a3b8" },
              ]}
            >
              <Text
                style={[
                  styles.catBannerText,
                  { color: activeCategory?.color ?? "#94a3b8" },
                ]}
              >
                {showGlobal
                  ? "🔎  SEARCH RESULTS"
          ...
          {showGlobal ? (
            ...
            <View style={styles.emptyState}>
              ...
              <Text style={styles.emptyText}>
                Select a category or search to browse
              </Text>
            </View>
          )}
        </View>
          ...
          >
            {recipeText &&
              formatRecipeText(recipeText).map((item, idx) => {
                if (item.type === "header") {
                  return (
                    <Text key={idx} style={styles.recipeHeader}>
                      {item.content}
                    </Text>
                  );
                  ...
                  return (
                    <Text key={idx} style={styles.recipeParagraph}>
                      {item.content}
                    </Text>
                  );
                }
...
const styles = StyleSheet.create({
  ...
  modalHeader: {
    ...
    paddingVertical: 16,
    borderBottomWidth: 1,
    borderBottomColor: "rgba(197,251,69,0.15)",
  },
  modalTitle: {
    color: ACCENT,
    fontFamily: MONO,
